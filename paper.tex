...
\begin{abstract}
  ...
  memory~(BROM) design with hybrid memory technologies. We demonstrate a method
...
While many design methods envolved with new memory technologies endeavor to
reduce the memory access latency of going off-chip, this work employs the hybrid
on-chip memory and reconfiguration to minimize off-chip bandwidth requirement.
For each level of memory, we select the memory technology that has the highest
provided bandwidth within given capacity. On top of such hardware hierarchy
design, we apply dynamic memory system reconfiguration that awares of the
bandwidth demand of a specific set of applications. 



Based on our memory technology exploration, we now provide an overview of our
bandwidth-aware architecture (Section~\ref{sec:hierarchy}) , and then discuss the details on the hardware
support (Section 3.2) and OS support (Section 3.3) required for achieving high
