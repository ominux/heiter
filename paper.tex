...
\begin{figure}[t]
\centering
\includegraphics[width=2.5in]{figures/overview}
\caption{Overview of BAROM hardware configuration.}
\label{fig:overview}
\end{figure}

Figure~\ref{fig:overview} depicts an overview of BAROM hardware configuration.
...
Figure~\ref{fig:overview}. The hardware and software supports for
...
throughput computing applications. Consequently, we define a lower bound of
memory capacity as $s_i^l=f^{-1}(\textrm{DBW}*(1+\sigma))$, where $\sigma$ is a
pre-defined threshold to make sure possible provided bandwidth increase without
much power overhead. The capacity of memory level-i ($s_i$) is therefore
selected to satisfy $s_i^l\le s_i\le s_i^u$. Furthermore, the rest of the
capacity can be reconfigured to become level-i.5 if necessary. We describe the
reconfiguration algorithm as follows.
...
Figure~\ref{fig:overview} shows the block diagram for the organization of our
...
speech and natural language processing~\cite{Chen:1996:ESS}. The n-gram models
...
\begin{figure*}[t]
\centering
\includegraphics[width=5in]{figures/predictor}
\caption{Components of prediction engine.}
\label{fig:predictor}
\end{figure*}
