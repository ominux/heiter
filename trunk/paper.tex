...
Figure~\ref{fig:hw-overview} shows an overview of BAROM hardware configuration.
The CMP platform consists of multiple cores, where L1 cahces are private to each
...
is selected. In our case, SRAM, STT-RAM, and RRAM are selected as the L2, L3, and
L4 caches respectively. As a result, the overall provided bandwidth curve of the
on-chip memory system will appear to be an envelope line of all the available
memory technologies as shown in Figure~\ref{fig:pbw-overall}.\vspace{5pt}
...
respectively.\vspace{5pt}

When the system is initially powered on, the capacity of each level is defined
to be the maximum available capacity of the level.
