...
the product of a sequence of conditional probabilities using the following
equation.
...
preceding $n-1$ samples and obtain Equation~\ref{eq:pr2}.

\begin{equation}
  p(s)=\prod_{i=1}^{l}p(w_i|w_{i-n+1}^{i-1})\label{eq:pr2}
\end{equation}
in which $w_{i-n+1}^{i-1}$ represents the sequence of $w_{i-n+1}...w_{i-1}$.
